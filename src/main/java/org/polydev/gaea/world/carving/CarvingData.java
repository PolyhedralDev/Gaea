...
public class CarvingData {
    ...
    private void isInRange(int x, int y, int z) {
        if(x > 15 || z > 15 || y > 255 || x < 0 || z < 0 || y < 0)
            throw new IllegalArgumentException("Value out of range! " + x + ", " + y + ", " + z);
    }
    ...
    public ChunkData merge(ChunkData data, boolean doLava) {
        for(Vector v : carvedBlocks.keySet()) {
            ...
            if(! m.equals(Material.BEDROCK) && m.isSolid())
                data.setBlock(v.getBlockX(), v.getBlockY(), v.getBlockZ(), (v.getBlockY() < 8 && doLava) ? Material.LAVA : Material.AIR);
        }
        return data;
    }
    ...
    public void merge(Chunk data, boolean doLava) {
        for(Vector v : carvedBlocks.keySet()) {
            ...
            if(! m.equals(Material.BEDROCK) && m.isSolid())
                data.getBlock(v.getBlockX(), v.getBlockY(), v.getBlockZ()).setType((v.getBlockY() < 8 && doLava) ? Material.LAVA : Material.AIR);
        }
    }
